...
**การขยายกลุ่มผู้ใช้แบบซื่อสัตย์ (Round 10, ตาม feedback ของ mentor):** เราไม่แต่งงานวิจัยผู้ใช้กลุ่มใหม่
แต่ยึดกับสิ่งที่ *มิติข้อมูลของตัวผลิตภัณฑ์เอง* ให้บริการได้จริงหลังรอบนี้ — เป็นข้อโต้แย้งเชิงความสามารถของ
ผลิตภัณฑ์ ไม่ใช่หลักฐานเชิงผู้ใช้ (ทั้งสองอย่างจริงพร้อมกัน): นักแปล/ผู้เรียน ได้ความหมาย + คำใกล้เคียง +
คำทับศัพท์ทางการ; ผู้เชี่ยวชาญเฉพาะทางได้ศัพท์บัญญัติ ๓ สาขาของตัวเอง; ผู้สนใจภาษา/ครูได้รากคำ +
วิวัฒนาการนิยาม ๓ ยุค — ทุกกลุ่มได้จากการ **ค้นคำเดียว เห็นทุกมิติในหน้าเดียว** (Phase U2). นี่จริงขึ้นตามตัวอักษร
หลัง Phase R/T/E/W ลงจริง โดยไม่ต้องอ้างงานวิจัยผู้ใช้ที่ยังไม่มี
...
**การจัดอันดับที่รันจริงหลัง Round 5 (ยืนยัน 2026-09-13):** คะแนนที่จัดอันดับคำที่เกี่ยวข้องคือ log-ratio
PPR ข้างต้น รันบนกราฟที่ผูกความสัมพันธ์ผ่าน "ชุดความหมาย" (sense group — ดูหมวด 6.6) โดยเฉพาะ:
- **Global PageRank `π`** คำนวณ **ครั้งเดียวตอน build** แล้ว cache ไว้ในโครงสร้าง engine (ไม่คำนวณซ้ำต่อ query)
- **Personalized PageRank `π_q`** เทเลพอร์ตไปที่คำ query แล้วให้คะแนน `log π_q − log π`
- **ตัวตัดสินเมื่อคะแนนเท่ากัน (tiebreak):** ความถี่จาก `tnc_freq.txt` — PPR มาก่อน, ความถี่ใช้เฉพาะตอน
  คะแนนใกล้เคียงกัน (ป้องกันคำโบราณ เช่น `คฤห`/`คฤหา` แซงคำสามัญ) จากนั้นเรียงตามตัวอักษรเพื่อความ deterministic
- **รันบนกราฟเต็ม ไม่ใช่ bounded/approximation** — วัด p95 warm latency ได้ **13.7 มิลลิวินาที** (ต่ำกว่า
  งบ 200 มิลลิวินาทีมาก) จึงไม่ต้องใช้ bounded local PPR (ถ้าจำเป็นในอนาคต fallback คือจำกัด power
  iteration ที่ BFS neighbourhood — เป็น approximation ที่จะระบุให้ชัดหากใช้ ตอนนี้ไม่ได้ใช้)

> **หมายเหตุประวัติ:** Round 5 Task 4 เคยแทนการจัดอันดับนี้ด้วยการนับเส้นเชื่อม (edge counting) โดยไม่แจ้ง
> ทำให้คะแนนกลายเป็นจำนวนเต็มและคำโบราณแซงคำสามัญ — Round 5B (A1) กู้ PPR กลับมาบนกราฟ sense-scoped
> เดิม โดยไม่ย้อน sense-scoping ทดสอบ `ranking_is_not_edge_count` + `lookup_uses_personalized_pagerank`
> ยืนยันว่ากลไก PPR ของ `graph.rs` ถูกเรียกจริงและคะแนนเป็นค่าต่อเนื่อง
...
### 6.6 Confidence Signal — จาก degree-based สู่ band-based (เรื่องที่เรากลับคำตามหลักฐาน)

**ปัญหาที่แก้:** ข้อมูล WordNet/Wiktionary ที่ import มาอัตโนมัติมี noise บางคู่ผิด ต้องการวิธีตั้งธง
"ควรตรวจ" โดยไม่ต้องมี machine learning ใดๆ

**ดีไซน์เดิม (Round 3–6) — Node-Degree Corroboration:** นับ degree ของแต่ละโหนด — ถ้าคู่คำ WordNet
**ทั้งสองฝั่ง degree = 1** (คู่โดดเดี่ยว ไม่มีชุดคำอื่นยืนยันซ้ำ) → ติดธง `Unverified`; มิฉะนั้น →
`Confirmed`. สัญชาตญาณคือ "ยิ่งมีชุดคำใหญ่ยืนยัน ยิ่งน่าเชื่อ"

**สิ่งที่วัดได้ แล้วพบว่าสัญชาตญาณกลับหัว (Round 6 Phase N audit, seed `0x4e362026`, n=40/tier):**

| กลุ่ม | ความแม่น (วัดมือ) |
|---|---|
| tier 2 หลายแหล่งตรงกัน | 92.5% |
| tier 3 ORST + tier 0 คู่โดดเดี่ยว | ~80–82% |
| **tier 1 synset แหล่งเดียว ≥3 คำ** | **55% ← ต่ำสุด** |

**คู่โดดเดี่ยว (ที่ธงเดิมเตือน) วัดได้ 80% ส่วน synset ใหญ่แหล่งเดียว (ที่ธงเดิมเงียบ) วัดได้แค่ 55%** —
ธงเดิมจึงเตือนกลุ่มที่ *ดีกว่า* และเงียบกับกลุ่มที่ *แย่กว่า* มันกลับหัว. เดิมยังวัด degree-signal โดยรวม
ได้ Confirmed 85.1% vs Unverified 81.8% ซึ่งแทบแยกไม่ออก — สอดคล้องกับข้อสรุปว่า degree ไม่ใช่สัญญาณที่ดี

**การแก้ (Round 7 T2):** ย้ายธงมาอิงกับ **band ที่วัดได้จริง** — เตือน **band C** (synset แหล่งเดียว
≥3 คำ, 55%) และเงียบกับ band A/B (≥80%). โค้ด: `classify_confidence_band()`. นี่คือการ *กลับคำตาม
หลักฐาน ไม่ใช่ตามสัญชาตญาณ* — เก็บ finding เดิมไว้ในเอกสารนี้ทั้งหมดเพราะการกลับหัวคือเรื่องเล่า ไม่ใช่
ความน่าอาย: ทีมที่ตรวจ heuristic ของตัวเองแล้วยอมแก้ตามตัวเลข คือสิ่งที่โปรเจกต์นี้ตั้งใจทำ

**ข้อควรระวังทางสถิติ (T3):** n=40/กลุ่ม → CI ~±10 pp ที่ 95%. ช่องว่าง **55% vs 80% แยกกันจริง**
(ต่างเกิน CI) ส่วน **92.5% vs 82.5% แยกกันไม่ชัด** (อยู่ใน CI ของกัน) — จึงยุบ tier 3 กับ tier 0 เป็น
band เดียว. และเลข 92.5% ครอบเฉพาะ **1,076 จาก 158,287 คู่ (0.68%)** ที่มีหลายแหล่งยืนยัน

---

## 7. ข้อมูล (Data) และสัญญาอนุญาต

**การเปลี่ยนลักษณะของกราฟ (corpus shift, วัด 2026-09-14):** กราฟความสัมพันธ์ตอนนี้ **~84% มาจาก
Wiktionary** — Wiktionary มีส่วนใน **132,631 จาก 158,045 คู่**; WordNet 26,225; ศัพท์บัญญัติ 120; seed 145.
มีเพียง **0.68%** (1,074 คู่) ที่มีหลายแหล่งยืนยันซ้ำ แปลว่า Thai WordNet กับ Wiktionary เก็บความรู้คำพ้อง
ที่ **แทบไม่ทับกัน** — การรวมสองแหล่งจึงเพิ่ม *ความครอบคลุม* มากกว่าเพิ่มความซ้ำ และคู่ที่บังเอิญตรงกัน
(band A) กลับแม่นสูงสุด (92.5%, ครอบเฉพาะ 0.68% นี้). ผลข้างเคียง: ผลลัพธ์ส่วนใหญ่ที่ผู้ใช้เห็นมาจาก
Wiktionary (community source) ไม่ใช่ WordNet — ป้ายที่มาจึงสำคัญ และเป็นเหตุผลที่เราติดป้ายทุกความสัมพันธ์
...
| รายการ | ผลลัพธ์ |
|---|---|
| จำนวนเทสที่ผ่าน | **85/85** (`wacha`) + 10/10 (`poc`) |
| Cold start (สร้าง trie จากคำ ~62k คำ + โหลดทุกแหล่ง ครั้งแรก) | **~60-62 วินาที** |
| Warm start (โหลดจาก cache) | **~1.5 วินาที** |
| p95 warm lookup latency (20 seed + 20 สุ่ม, ยิงผ่าน HTTP server) | **13.7 มิลลิวินาที** |
| จำนวนโหนด/เส้นเชื่อม/sense node ในกราฟความสัมพันธ์ | **57,061 entities / 74,215 triples / 29,759 sense nodes** |
| คำที่ค้นได้ (segmenter) / entry ที่มีนิยาม | 72,175 คำ / 29,601 entries |
| ครอบคลุมนิยาม — ดิบ | 29,601 / 72,175 = **41.0%** |
| ครอบคลุมนิยาม — ถ่วงความถี่ (top-100 / top-1000 / top-5000) | **100% / 93.9% / 81.3%** (ระบุตัวส่วนเสมอ) |
| ลิงก์ข้ามความหมายผิด (cross-sense 2-hop) — ก่อน → หลัง sense-scoping | **150,018 → 0** (มีเทสต์ `no_cross_sense_two_hop_pairs` นับสดจากกราฟ) |
| Recall/absence บนคู่ที่ตรวจมือ 47 คู่ (ก่อน c733acc → หลัง HEAD) | KEEP-recall 97.5% → **100%** · CUT-absence **100%** ทั้งสอง || ความแม่นยำของความสัมพันธ์จาก WordNet (สุ่มตรวจ 120 คู่จริง) | **84.2%** (Confirmed tier 85.1%, Unverified tier 81.8%) — ครอบคลุมเฉพาะชั้น WordNet ที่แสดง (1-hop) |
| การทดสอบ adversarial (ยิงจริง 11 เคส: ว่าง, ยาว 5000 ตัวอักษร, emoji, UTF-8 ผิด, XSS injection ฯลฯ) | **ผ่านหมด ไม่ crash ไม่ค้าง** (ยิงซ้ำ Round 5B) |
| ตรวจ XSS ที่ชั้น frontend จริง (ไม่ใช่แค่ชั้น API) | ทุกจุดที่แทรก user input ผ่านฟังก์ชัน `esc()` จริง ไม่มีช่องโหว่ |
| ศัพท์บัญญัติ (ORST) demo subset | 39 คำอังกฤษ, `field` → 8 สาขาวิชา; ทุกคู่ติดป้าย CoinedWord/Confirmed จัดอันดับเหนือ WordNet |
| Git/repo | `wacha` build ได้โดยไม่พึ่ง repo ภายนอกเลย; `katgpt-rs` (repo ที่ไม่ใช่ของโปรเจกต์นี้) ไม่ถูกแตะต้องเลยตลอดกระบวนการ |

**ขอบเขตของ KEEP-recall 40/40 (พูดตามจริง):** ตัวเลขนี้วัดบน **47 คู่ที่อยู่ติดกับ seed** เท่านั้น
(`data/seed_wordnet_audit_2026-09-13.md`) จึง **ไม่ครอบคลุม** การสูญเสีย recall นอกชุดนี้ — เช่น
`รถยนต์ → ยานยนต์` ที่ถูกตัดทิ้ง (อยู่คนละ synset จึงเป็น 2-hop ข้ามความหมายที่ตัดถูกต้อง แต่สองคำนี้ใกล้กัน
จริงในภาษาไทย) พูดง่าย ๆ: 40/40 = "ของคู่ seed ที่เราตรวจมือ ไม่มีตัวไหนหาย" ไม่ใช่ "ระบบไม่เคยตัดคำพ้อง
จริงทิ้งเลย" (ดู `rounds/VERIFY_R5.md` §2 สำหรับ before/after เต็ม)
...
5. ความสัมพันธ์จาก WordNet บางคู่ผิดจริง (เช่น ข้อหา/มลทิน) — แก้ด้วยระบบติดป้ายความน่าเชื่อถือ ไม่ใช่การ
   ลบทิ้งทั้งหมด
6. (Round 5) การผูกความสัมพันธ์แบบ flat word↔word สร้างลิงก์ข้ามความหมาย 150,018 คู่ (ครอบครัว→บ้าน→
   บ้านเกิด) — แก้ที่โครงสร้างด้วย sense group เหลือ 0 คู่ (มีเทสต์นับสด)
7. (Round 5 Task 4 → แก้ใน 5B) การ sense-scoping เผลอแทน Personalized PageRank ด้วยการนับเส้นเชื่อม
   โดยไม่แจ้ง — คะแนนกลายเป็นจำนวนเต็ม, คำโบราณแซงคำสามัญ, และ PITCH อธิบายอัลกอริทึมที่ไม่ได้รัน
   รีวิวจับได้ กู้ PPR กลับ (A1) พร้อมเทสต์ยืนยันว่า `graph.rs` ถูกเรียกจริงและคะแนนต่อเนื่อง
