...
// Prebuilt global PageRank vector (W3): embedded so `wacha_init` loads it in µs
// instead of recomputing (~8 s) — `Instant` also can't run on wasm anyway.
// Generated by `wacha/examples/gen_pagerank_blob.rs` (keyed on the graph hash).
const PAGERANK_BLOB: &[u8] = include_bytes!("../assets/pagerank.blob");
    ...
    ENGINE.with(|cell| {
        ...
        if opt.is_none() {
            ...
            let eng = match wacha::segmenter::Segmenter::from_cache_bytes(SEG_CACHE) {
                Ok(seg) => Engine::build_from_segmenter_pr_bytes(words, entries, None, seg, PAGERANK_BLOB),
